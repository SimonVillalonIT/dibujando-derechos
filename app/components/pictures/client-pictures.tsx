...
function ClientPictures({ draws }: { draws: Draw[] }) {
  ...
  return (
    <div
      className={`grid grid-cols-3 min-h-96 gap-16 transition-opacity duration-500 ${
        isFading ? "opacity-0" : "opacity-100"
      }`}
    >
      {currentDraws?.map((draw) => (
        <DrawCard school_name="Pre-Seleccionados" draw={draw} key={draw.id} setSchoolData={setCurrentDraws} />
      ))}
    </div>
  );
}
